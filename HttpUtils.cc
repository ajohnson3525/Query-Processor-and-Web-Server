...
bool IsPathSafe(const string& root_dir, const string& test_file) {
  ...
  char absolute_path[PATH_MAX * 3];
  const char* tf = test_file.c_str();
  const char* rd = root_dir.c_str();
  char rd_slash[strlen(rd)];
  strcpy(rd_slash, rd);
  strcat(rd_slash, "/");
  realpath(tf, absolute_path);
  return strstr(absolute_path, rd_slash) != NULL;
}
